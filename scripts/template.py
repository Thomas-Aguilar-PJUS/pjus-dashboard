...
function rPipe(){{
  ...
  const maxVal=Math.max(...pipes.map(p=>p.val),1);dc('cPB');ch['cPB']=new Chart(document.getElementById('cPB'),{{type:'bubble',data:{{datasets:[{{data:pipes.map(p=>({{x:p.dias,y:p.score,r:Math.max(5,Math.min(35,Math.sqrt(p.val/maxVal)*35))}})),backgroundColor:pCo.map(c=>c+'55'),borderColor:pCo,borderWidth:1.5}}]}},options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}},tooltip:{{callbacks:{{label:c=>pipes[c.dataIndex].fase+' — '+Fv(pipes[c.dataIndex].val)}}}}}},scales:{{x:{{title:{{display:true,text:'Dias'}},grid:{{color:'rgba(224,231,245,.4)'}}}},y:{{title:{{display:true,text:'Score'}},min:0,max:5.5,grid:{{color:'rgba(224,231,245,.4)'}}}}}}}}}});
